...
class AccountInvoiceReport(models.Model):
    ...
    company_price_subtotal = fields.Monetary(
        currency_field="current_currency_id",
        string="Untaxed Total (CC)",
        help="Untaxed Total in the company's currency where it is set",
    )
    company_price_total = fields.Monetary(string="Total", currency_field="currency_id")
    company_price_average = fields.Monetary(
        currency_field="current_currency_id",
        string="Average Price (CC)",
        help="Average Price in the company's currency where it is set",
    )
    company_price_margin = fields.Float(string="Margin (CC)", help="Margin in the company's currency where it is set")
    ...
    company_price_subtotal_currency = fields.Monetary(string="Untaxed Amount in Currency", currency_field="currency_id")
